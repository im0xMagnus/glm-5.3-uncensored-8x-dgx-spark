...
for ip in "$@"; do
  scp -q -o BatchMode=yes prism-spark-ops.sudoers "$USER@10.100.128.$ip:/tmp/prism-spark-ops.sudoers"
  r=$(printf '%s\n' "$SUDO_PW" | ssh -o BatchMode=yes -o ConnectTimeout=15 "$USER@10.100.128.$ip" '
    out=$(sudo -S -p "" sh -c "visudo -cf /tmp/prism-spark-ops.sudoers >/dev/null 2>&1 && install -o root -g root -m 0440 /tmp/prism-spark-ops.sudoers /etc/sudoers.d/prism-spark-ops 2>&1 && echo INSTALLED" 2>&1 | tail -1)
