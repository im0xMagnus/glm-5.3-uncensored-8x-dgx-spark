#!/bin/bash
# Runs ON NODE_HOST_2_PLACEHOLDER (detached, immune to the Mac). Same measurements we trust from FP8,
...
log "=== 0. smoke ==="
curl -sf -m 90 http://NODE_PREFIX_PLACEHOLDER.10:8888/v1/chat/completions -H 'Content-Type: application/json' \
...
log "  acceptance: $(ssh -o BatchMode=yes -o ConnectTimeout=10 $USER@NODE_PREFIX_PLACEHOLDER.10 'docker logs vllm_nvfp4_tp8 2>&1 | grep -a "Mean acceptance length" | tail -1 | grep -oE "Mean acceptance length: [0-9.]+"' 2>/dev/null)"
