#!/bin/bash
# GLM-5.3-UNCENSORED-NVFP4 . TP=8 . PATH B-PRIME: fp8 KV + DCP8 -> multiple concurrent 1M sessions
# (nvfp4 KV + DCP8 failed 2026-09-08: indexer k_cache page size not divisible by the nvfp4 main-KV page size)
# Derived from the validated launch-nvfp4-tp8.sh (2026-09-08); exactly five flag-line changes:
#   --kv-cache-dtype fp8_ds_mla        (unchanged; DCP supplies the capacity, density not needed)
#   --max-model-len 1048576
#   --decode-context-parallel-size 8   (one sequence's latent KV split across the 8 ranks)
#   VLLM_DCP_SHARD_DRAFT=0             (MTP draft stays unsharded under DCP -- MTP stays ON)
#   NCCL_MAX/MIN_NCHANNELS 4 -> 2      (ciprianveg's DCP8 recipe; revisit as its own variable)
# Unchanged: gmu 0.80, seqs 4, MTP k=1, graphs FULL, `low`. Container name unchanged so tooling works.
# UNPROVEN on GLM-5.3: DCP8 + DSA sparse indexer. Expect ~19-20 tok/s shallow, faster at depth.
set -u
...
docker run --gpus all -d --name "$NAME" --restart no \
  ...
  -e NCCL_MAX_NCHANNELS=2 -e NCCL_MIN_NCHANNELS=2 \
  ...
  "$IMAGE" \
    ...
    --decode-context-parallel-size 8 --dcp-kv-cache-interleave-size 1 \
