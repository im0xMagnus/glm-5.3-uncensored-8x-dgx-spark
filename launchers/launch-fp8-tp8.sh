#!/bin/bash
# GLM-5.3-UNCENSORED-FP8 · TP=8 · 8x DGX Spark GB10
#
# usage: ./launch-fp8-tp8.sh <node-rank 0-7>
#   rank 0 = head (serves the API), ranks 1-7 = --headless workers.
#   Launch WORKERS FIRST (7,6,5,4,3,2,1) then the head (0).
#
# Derived from ciprianveg/gb10-vllm glm-5.3/v19 (the proven TP=8 recipe for the
# full GLM-5.3), with the executor pattern from tonyd2wild's TP=4 launcher
# (`mp` + --nnodes/--node-rank) because it needs no Ray cluster.
#
# EVERY DEVIATION FROM THE UPSTREAM RECIPE IS MARKED "DELTA:" WITH ITS REASON.
set -u
...
IMAGE="ghcr.io/ciprianveg/gb10-glm-5.2:v19-vision"
NAME="vllm_fp8_tp8"
MODEL_HOST=/data/models/GLM-5.3-UNCENSORED-FP8      # local copy on every node
MODEL_PATH=/models/glm-5.3-fp8
CACHE_HOST=/var/tmp/fp8-vllm-cache
